...
import com.ysydhclib.framelib.FrameImageUtils
import com.ysydhclib.framelib.R
import java.io.IOException

/**
 * Created by leafye on 2019/3/21.
 */
class FrameAnimView : View, GestureDetector.OnGestureListener, View.OnTouchListener {
    /**
     * 帧动画视图，可以播放逐帧动画,
    ...
    private val scaleTypeArray by lazy {
        arrayOf(ImageView.ScaleType.FIT_CENTER, ImageView.ScaleType.CENTER, ImageView.ScaleType.FIT_XY)
    }
    ...
    interface FramePlayListener {
        fun framePlay(view: FrameAnimView, frameIndex: Int, framePlayState: FramePlayState?)
    }
    ...
    constructor(
        ...
        FrameAnimView.frameDuration = frameDuration
    ...
    fun isFingerGestureEnable():Boolean{
        return this.fingerGestureEnable
    }
        ...
        try {
            if (foreUpdateUI || bitmapIndex != preFrame || bitmap == null) {
                ...
                bitmap = FrameImageUtils.getAssetBitmap(
                    context, imagePath, 1,
                    if (bitmapConfig != null) bitmapConfig else Bitmap.Config.RGB_565
                )
                resetMatrix(bitmap)
